...
footer 按钮显示一个状态点：绿色表示已是最新，检查发现落后于上游的 commit 时为琥珀色，任务运行时为蓝色。打开面板会加载 status 与当前任务（如果有）；第一行始终显示已检出的发布版本与 `HEAD` 短 sha。「Check」会 fetch 上游并报告该 checkout 落后多远。「Update」会立即启动一个任务——没有确认步骤，也不会询问关于活动 Session 的任何事情：Host 会自行 flush 并透明恢复任何进行中的轮次。被 Host 拒绝的启动（已有另一个任务在运行、工作树不干净、没有重启能力）会在面板中点名，失败的 Remote 调用会显示其消息，因此一次点击绝不会看起来什么都没发生。

### 观察进度并在重启后恢复

任务运行期间，面板显示其当前 phase 以及底层 git/install/build/verify 输出的滚动日志；在任务结束之前，它会保持打开，无法通过点击外部或 Escape 关闭，并且会在任务变为活动状态的那一刻自行打开——即使这个任务是从另一个标签页启动的——因此实时进度绝不会因为一次意外关闭而丢失。任务进入重启阶段后，面板切换为「Restarting…」横幅；此后物理连接重新建立时，页面会自动重新加载，确保每个 client bundle 都是最新的。
